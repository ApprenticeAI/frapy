...
def collect_frapy_settings_refs(app, doctree):
    env = app.builder.env

    if not hasattr(env, "frapy_all_settings"):
        env.frapy_all_settings = []
    ...
    for node in doctree.traverse(settingslist_node):
        ...
        settings_list.extend(
            [
                ...
                for d in sorted(env.frapy_all_settings, key=itemgetter("setting_name"))
...
def setup(app):
    ...
    app.connect("doctree-read", collect_frapy_settings_refs)
    app.connect("doctree-resolved", replace_settingslist_nodes)


def source_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ref = "https://github.com/frapy/frapy/blob/master/" + text
    set_classes(options)
...
def issue_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ref = "https://github.com/frapy/frapy/issues/" + text
    set_classes(options)
...
def commit_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ref = "https://github.com/frapy/frapy/commit/" + text
    set_classes(options)
...
def rev_role(name, rawtext, text, lineno, inliner, options={}, content=[]):
    ref = "http://hg.frapy.org/frapy/changeset/" + text
    set_classes(options)
