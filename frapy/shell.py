...
from frapy.utils.conf import get_config
from frapy.utils.console import DEFAULT_PYTHON_SHELLS, start_python_console
from frapy.utils.datatypes import SequenceExclude
from frapy.utils.misc import load_object
from frapy.utils.reactor import is_asyncio_reactor_installed, set_asyncio_event_loop
...
class Shell:
    ...
    def start(self, url=None, request=None, response=None, spider=None, redirect=True):
        ...
        if self.code:
            ...
            """
            Detect interactive shell setting in frapy.cfg
            e.g.: ~/.config/frapy.cfg or ~/.frapy.cfg
            [settings]
    ...
    def get_help(self):
        ...
        if self.inthread:
            ...
            b.append(
                "  fetch(req)                  "
                "Fetch a frapy.Request and update local objects "
            )
