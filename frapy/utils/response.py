...
from frapy.utils.decorators import deprecated
from frapy.utils.python import to_bytes, to_unicode

_baseurl_cache: "WeakKeyDictionary[Response, str]" = WeakKeyDictionary()


def get_base_url(response: "frapy.http.response.text.TextResponse") -> str:
...
def get_meta_refresh(
    response: "frapy.http.response.text.TextResponse",
    ignore_tags: Iterable[str] = ("script", "noscript"),
) -> Union[Tuple[None, None], Tuple[float, str]]:
    """Parse the http-equiv refresh parameter from the given response"""
    if response not in _metaref_cache:
        text = response.text[0:4096]
        # a w3lib typing bug here, fixed in https://github.com/frapy/w3lib/pull/211
...
def open_in_browser(
    response: Union[
        "frapy.http.response.html.HtmlResponse",
        "frapy.http.response.text.TextResponse",
    ],
    ...
    """
    from frapy.http import HtmlResponse, TextResponse
