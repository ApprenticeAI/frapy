...
def configure_logging(settings=None, install_root_handler=True):
    ...
    :type settings: dict, :class:`~frapy.settings.Settings` object or ``None``
    ...
    if install_root_handler:
        install_frapy_root_handler(settings)


def install_frapy_root_handler(settings):
    global _frapy_root_handler

    if (
        _frapy_root_handler is not None
        and _frapy_root_handler in logging.root.handlers
    ):
        logging.root.removeHandler(_frapy_root_handler)
    logging.root.setLevel(logging.NOTSET)
    _frapy_root_handler = _get_handler(settings)
    logging.root.addHandler(_frapy_root_handler)


def get_frapy_root_handler():
    return _frapy_root_handler


_frapy_root_handler = None
...
def _get_handler(settings):
    ...
    if settings.getbool("LOG_SHORT_NAMES"):
        handler.addFilter(TopLevelFormatter(["frapy"]))
    return handler
