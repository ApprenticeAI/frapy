...
from frapy.signalmanager import SignalManager
from frapy.utils.log import (
    LogCounterHandler,
    configure_logging,
    get_frapy_root_handler,
    install_frapy_root_handler,
    log_reactor_info,
    log_frapy_info,
)
from frapy.utils.misc import create_instance, load_object
from frapy.utils.ossignal import install_shutdown_handlers, signal_names
...
if TYPE_CHECKING:
    from frapy.utils.request import RequestFingerprinter
...
class Crawler:
    def __init__(self, spidercls, settings=None, init_reactor: bool = False):
        ...
        if get_frapy_root_handler() is not None:
            # frapy root handler already installed: update it with new settings
            install_frapy_root_handler(self.settings)
...
class CrawlerRunner:
    ...
    crawlers = property(
        ...
        doc="Set of :class:`crawlers <frapy.crawler.Crawler>` started by "
    ...
    def crawl(self, crawler_or_spidercls, *args, **kwargs):
        ...
        If ``crawler_or_spidercls`` isn't a :class:`~frapy.crawler.Crawler`
        ...
        :type crawler_or_spidercls: :class:`~frapy.crawler.Crawler` instance,
            :class:`~frapy.spiders.Spider` subclass or string
    ...
    def create_crawler(self, crawler_or_spidercls):
        """
        Return a :class:`~frapy.crawler.Crawler` object.
...
class CrawlerProcess(CrawlerRunner):
    """
    A class to run multiple frapy crawlers in a process simultaneously.

    This class extends :class:`~frapy.crawler.CrawlerRunner` by adding support
    for starting a :mod:`~twisted.internet.reactor` and handling shutdown
    signals, like the keyboard interrupt command Ctrl-C. It also configures
    top-level logging.

    This utility should be a better fit than
    :class:`~frapy.crawler.CrawlerRunner` if you aren't running another
