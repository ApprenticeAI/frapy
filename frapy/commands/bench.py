...
class _BenchServer:
    def __enter__(self):
        ...
        pargs = [sys.executable, "-u", "-m", "frapy.utils.benchserver"]
...
class _BenchSpider(frapy.Spider):
    ...
    def start_requests(self):
        ...
        return [frapy.Request(url, dont_filter=True)]

    def parse(self, response):
        for link in self.link_extractor.extract_links(response):
            yield frapy.Request(link.url, callback=self.parse)
