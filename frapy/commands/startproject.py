...
class Command(ScrapyCommand):
    ...
    def _copytree(self, src: Path, dst: Path):
        ...
        More info at:
        https://github.com/frapy/frapy/pull/2005
        """
        ignore = IGNORE
    ...
    def run(self, args, opts):
        ...
        if (project_dir / "frapy.cfg").exists():
            self.exitcode = 1
            print(f"Error: frapy.cfg already exists in {project_dir.resolve()}")
            return
        ...
        print("    frapy genspider example example.com")

    @property
    def templates_dir(self) -> str:
        return str(
            Path(
                self.settings["TEMPLATES_DIR"] or Path(frapy.__path__[0], "templates"),
                "project",
            )
        )
