...
import frapy
from frapy.http.common import obsolete_setter
from frapy.http.headers import Headers
from frapy.utils.curl import curl_to_request_kwargs
from frapy.utils.python import to_bytes
from frapy.utils.trackref import object_ref
from frapy.utils.url import escape_ajax
...
def NO_CALLBACK(*args, **kwargs):
    ...
    :class:`~frapy.http.Request`, it indicates that the request is not meant
    ...
    :meth:`frapy.core.engine.ExecutionEngine.download`, so that downloader
    middlewares handling such requests can treat them differently from requests
    intended for the :meth:`~frapy.Spider.parse` callback.
    """
    raise RuntimeError(
...
class Request(object_ref):
    ...
    :func:`~frapy.utils.request.request_from_dict`.
    """
    ...
    def from_curl(
        ...
        **kwargs,
        ...
        .. caution:: Using :meth:`from_curl` from :class:`~frapy.http.Request`
                     subclasses, such as :class:`~frapy.http.JSONRequest`, or
                     :class:`~frapy.http.XmlRpcRequest`, as well as having
                     :ref:`downloader middlewares <topics-downloader-middleware>`
                     and
                     :ref:`spider middlewares <topics-spider-middleware>`
                     enabled, such as
                     :class:`~frapy.downloadermiddlewares.defaultheaders.DefaultHeadersMiddleware`,
                     :class:`~frapy.downloadermiddlewares.useragent.UserAgentMiddleware`,
                     or
    ...
    def to_dict(self, *, spider: Optional["frapy.Spider"] = None) -> dict:
        """Return a dictionary containing the Request's data.

        Use :func:`~frapy.utils.request.request_from_dict` to convert back into a :class:`~frapy.Request` object.
